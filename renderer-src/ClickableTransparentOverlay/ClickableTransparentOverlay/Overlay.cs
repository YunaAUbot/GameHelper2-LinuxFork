namespace ClickableTransparentOverlay
{
    ...
    public abstract class Overlay : IDisposable
    {
        ...
        public Overlay(string windowTitle, bool DPIAware, int windowWidth, int windowHeight)
        {
            ...
            // Native GLX is an explicit Wine-only backend. Native Windows and
            // ordinary upstream consumers retain the HWND presenter.
            this.useNativeGpu = AppContext.BaseDirectory.StartsWith("Z:\\", StringComparison.OrdinalIgnoreCase) && string.Equals(
                Environment.GetEnvironmentVariable("GAMEHELPER2_OVERLAY_BACKEND"),
                "native-gpu",
                StringComparison.OrdinalIgnoreCase);
            LogRenderer($"configured backend={(this.useNativeGpu ? "native-gpu" : "windows")}");
        ...
        private void RunInfiniteLoop(CancellationToken token)
        {
            ...
            {
                if (this.useNativeGpu) NativeGpuProbe.Pulse(this.window.Dimensions);
                ...
                if (NativeGpuDisconnectPolicy.ShouldWaitForReconnect(this.useNativeGpu, this.nativeGpuStarted, NativeGpuProbe.IsConnected))
                {
                    ...
                    {
                        NativeGpuProbe.PrepareReconnect(this.inputhandler, this.renderer);
                ...
                if (this.useNativeGpu && !this.nativeGpuStartAttempted)
                {
                    ...
                    this.nativeGpuStarted = NativeGpuProbe.TryStart(this.window.Dimensions);
                    if (this.nativeGpuStarted)
                    {
                        User32.ShowWindow(this.window.Handle, ShowWindowCommand.Hide);
                        LogRenderer($"native GPU helper started at {this.window.Dimensions.X},{this.window.Dimensions.Y} {this.window.Dimensions.Width}x{this.window.Dimensions.Height}");
                    }
                    else
                    {
                        LogRenderer("native GPU helper unavailable; closing instead of exposing the opaque Wine window");
                        this.Close();
                        break;
                    }
                }
                var wantsMouse = this.inputhandler.Update(this.nativeGpuStarted);
                if (!this.nativeGpuStarted) Utils.SetOverlayClickable(this.window.Handle, wantsMouse);
                this.renderer.Update(currentTimeSec, () => { Render(); });
                if (this.nativeGpuStarted)
                {
                    NativeGpuProbe.SetInteractive(wantsMouse);
                    NativeGpuProbe.SetKeyboardCapture(this.inputhandler.WantsKeyboardCapture());
        ...
        private static void LogRenderer(string message)
        {
            ...
            {
                File.AppendAllText(
                    "GameHelper2.renderer.log",
                    $"{DateTimeOffset.Now:O} {message}{Environment.NewLine}");
            }
            catch
            {
                // Diagnostics must never make the renderer fail.
            }
        }
