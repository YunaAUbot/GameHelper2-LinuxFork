...
grep -q 'Guid.NewGuid():N.*Environment.ProcessId.*\.alive' "$CTO/NativeGpuProbe.cs" || fail "native restarts reuse one heartbeat marker and can retain an old compositor"
